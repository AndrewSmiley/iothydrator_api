...
def predix_omt():
    ...
    r = requests.post("https://time-series-store-predix.run.aws-usw02-pr.ice.predix.io/v1/datapoints", headers=predix.authorization_headers, json=data)
    #it's all in my head
    # bucket = {key: [] for key in reversed(range(23, -1, -1))}
    bucket= OrderedDict()
        ...
        for collection in json.loads(r.text)['tags'][0]['results']:
            if int(collection['groups'][0]['group']['keg']) != Keg.objects.last().id:
                ...
                for dp in collection['values']:
                    ...
                    # print dp[0]
                    bucket[datetime.datetime.utcfromtimestamp(int(dp[0]/1000)).hour].append(dp[1])
                    # total_volume = total_volume+dp[1]
    ...
    for k,v in bucket.iteritems():
        if len(v) == 0:
            hours.append(datetime.datetime.now().replace(hour=k))
            # if k > 12:
            #     pm_hours.append("%sPM"%(k-12))
            # elif k < 12:
            #     am_hours.append("%sAM"%(k))
            # elif k ==12:
                # pm_hours.append("%sPM"%(k))
    # hours.sort(reverse=True)
    # am_hours.sort()
    # pm_hours.sort(reverse=True)
    hours.sort()
    sub_hours =[]
    for hour in hours:
        try:
            if hour.hour == sub_hours[-1][-1].hour +1:
                sub_hours[-1].append(hour)
            else:
                sub_hours.append([hour])
        except:
            sub_hours.append([hour])
    sub_hours.sort(key=lambda x: len(x))
    return sub_hours[-1]



datetime.datetime.utcfromtimestamp(float(1488492645160/1000))